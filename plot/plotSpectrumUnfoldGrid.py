...
eigValGenRng = np.ma.masked_all((nxRng.shape[0], nev))
diffRng = np.ma.masked_all((nxRng.shape[0], nev))
for inx, nx in enumerate(nxRng):
    print('nx = ', nx)
    gridPostfix = ''
    N = 1
    for d in np.arange(dimObs):
        ...
        gridPostfix = '{}_n{:d}l{:d}h{:d}'.format(
            gridPostfix, nx, int(cfg.grid.nSTDLow[d]),
    ...
    eigValGenRng[inx, :len(eigValGen)] = eigValGen.real

fig, ax = plt.subplots()
for ev in range(nevPlot):
    ax.plot(nxRng, eigValGenRng[:, ev])
ax.set_xlim(*nxRng[[0, -1]])
ax.set_ylim(xlimEig)
ax.vlines([100.], *xlimEig, linestyle='--', linewidth=1, colors='k')
ax.set_ylabel(realLabel, fontsize=ergoplot.fs_latex)
ax.set_xlabel(r'$n_x$', fontsize=ergoplot.fs_latex)
fileName = 'eigval_grid_nev{:d}{}.png'.format(nev, dstPostfixTau)
...
for ev in range(1, nevPlot):
    w = eigValGenRng[:, ev]
    diff = (w[:-1] - w[1:]) / w[:-1] * 100
    diffRng[1:len(diff) + 1, ev] = diff
    ax.plot(nxRng[1:], diff)
ax.set_xlim(*nxRng[[0, -1]])
ylim = [0., 20.]
ax.set_ylim(*ylim)
ax.hlines([1.], *nxRng[[0, -1]], linestyle='--', linewidth=1, colors='k')
ax.vlines([100.], *ylim, linestyle='--', linewidth=1, colors='k')
ax.set_ylabel(r'$\Delta \Re(\lambda_k)$ (%)', fontsize=ergoplot.fs_latex)
ax.set_xlabel(r'$n_x$', fontsize=ergoplot.fs_latex)
fileName = 'eigval_griddiff_nev{:d}{}.png'.format(nev, dstPostfixTau)
filePath = os.path.join(specDir, 'bootstrap', fileName)
fig.savefig(filePath, dpi=ergoplot.dpi, bbox_inches=ergoplot.bbox_inches)
