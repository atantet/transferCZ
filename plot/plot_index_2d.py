...
field_h = (1, 'Thermocline depth', r'h', r'$m$', H, r'$h^2$')
...
#        srcPostfix = '_mu%03d_eps%03d' \
#                     % (np.round(mu * 100, 1), np.round(eps * 1000, 1))
        ...
        # Read dataset
        indexData = np.loadtxt('%s/%s.txt' % (indicesPath, indexDef1[1]))
        timeND1 = indexData[:, 0]
        timeFull1 = timeND1 * timeDim / 365
        indexFull1 = indexData[:, fieldDef1[0]] * fieldDef1[4]
        indexData = np.loadtxt('%s/%s.txt' % (indicesPath, indexDef2[1]))
        timeND2 = indexData[:, 0]
        timeFull2 = timeND2 * timeDim / 365
        indexFull2 = indexData[:, fieldDef2[0]] * fieldDef2[4]
...
#        nt = time.shape[0]
        ...
        ax.plot(indexFull1[timeWin[0]:timeWin[1]], indexFull2[timeWin[0]:timeWin[1]],
                 linewidth=linewidth)
#        plt.title('Time-series of %s averaged over %s\nfor mu = %.1f and eps = %.2f' % (fieldDef[1], indexDef[0], mu, eps))
        ax.set_xlabel('%s %s (%s)' % (indexDef1[0], fieldDef1[1], fieldDef1[3]),
                      fontsize=fs_latex)
        ax.set_ylabel('%s %s (%s)' % (indexDef2[0], fieldDef2[1], fieldDef2[3]),
                      fontsize=fs_latex)
        ...
        fig.savefig('%s/%s%s%s%s%s.%s' % (pltDir, indexDef1[1], fieldDef1[2], indexDef2[1], fieldDef2[2],
                                      dstPostfix, figFormat), bbox_inches='tight', dpi=dpi)
