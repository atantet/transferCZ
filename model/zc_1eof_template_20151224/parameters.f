...
! Read experiment design
      ...
      READ(70, 40) DSTR, seed ! Noise level
      CLOSE(70)
 30   FORMAT(A6, F18.10)
 40   FORMAT(A6, A1)
...
*** read_tau is merged with force in order to read 
         if (k_solve.EQ.1) then
*     Read record of residual red-noise wind stress
            print *, "Openning forcing records..."
            print *, "seed = ", seed
            path = '../init_seed'//seed//
     &           '/tauWhite_1eofs_amp30_seed'//seed//'.bin'
            open (134, file=path,
     &           status='old', form='unformatted')
            path = '../init_seed'//seed//
     &           '/dtaudxWhite_1eofs_amp30_seed'//seed//'.bin'
            open (135, file=path,
     &           status='old', form='unformatted')
            path = '../init_seed'//seed//
     &           '/dtaudyWhite_1eofs_amp30_seed'//seed//'.bin'
            open (136, file=path,
     &           status='old', form='unformatted')
         endif
...
*     of the artificial meridional one and the stochastic residual one
         print *, 'Reading F0_t at ', k_solve, ' to taux'
         read (134) F0_t
         read (135) F0_t_dx
         read (136) F0_t_dy
         DO mx = 1, nx+1
            DO my = 1, ny+1
               taux(mx, my, 0) = taux(mx,my,0) + F0_t(mx, my)*par(31)
               taux(mx, my, 1) = taux(mx,my,1) + F0_t_dx(mx, my)*par(31)
               taux(mx, my, 2) = taux(mx,my,2) + F0_t_dy(mx, my)*par(31)
            END DO
         END DO
