...
for k in np.arange(nEOF):
    ...
    dtaudy[:, 0] = (tau[:, 1] - tau[:, 0]) / dY3Left
    dtaudy[:, -1] = (tau[:, -1] - tau[:, -2]) / dY3Right
    f = FortranFile('%s/tau_%deofs_ampMean%02d%s.bin' % (initDir, k+1, int(ampMean * 10), postfix), 'w')
    for t in np.arange(T):
        f.write_record(tau[t])
    f.close()
    f = FortranFile('%s/dtaudx_%deofs_ampMean%02d%s.bin' % (initDir, k+1, int(ampMean * 10), postfix), 'w')
    for t in np.arange(T):
        f.write_record(dtaudx[t])
    f.close()
    f = FortranFile('%s/dtaudy_%deofs_ampMean%02d%s.bin' % (initDir, k+1, int(ampMean * 10), postfix), 'w')
    for t in np.arange(T):
        f.write_record(dtaudy[t])
    f.close()
