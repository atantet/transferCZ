...
ssta = np.copy(sstMasked)
Wua = np.copy(WuMasked)
...
plt.title('Mean of the original wind-stress')
map.drawcoastlines()
# draw parallels and meridians.
map.drawparallels(np.arange(0, 81.,10.))
map.drawmeridians(np.arange(-180.,181.,30.))
plt.colorbar(orientation='horizontal')
fig.savefig('tau_orig_mean_%s.png' % postfix, bbox_inches='tight') 

# Plot std Wu
fig = plt.figure()
field = np.ma.masked_all((nlat*nlon,))
field[~mask] = Wua.std(0)
field = field.reshape(nlat, nlon)
vmax = np.max(field)
vmin = 0.
levels = np.linspace(vmin, vmax, nlev)
cs = map.contourf(x, y, field, levels, cmap=cm.hot_r)
plt.title('Std of the original wind-stress without seasonal cycle')
map.drawcoastlines()
# draw parallels and meridians.
map.drawparallels(np.arange(0, 81.,10.))
map.drawmeridians(np.arange(-180.,181.,30.))
plt.colorbar(orientation='horizontal')
fig.savefig('tau_orig_std_%s.png' % postfix, bbox_inches='tight') 

# Regressions
print 'Getting wind stress residuals...'
WuExtMeanDim = np.empty((nValid,))
WuResDim = np.copy(Wua)
for ij in np.arange(nValid):
    X = np.matrix(np.ones((nt, 2)))
    X[:, 0] = np.matrix(ssta[:, ij]).T
    Y = np.matrix(Wua[:, ij]).T
    A = (X.T * X)**(-1) * (X.T * Y)
    WuExtMeanDim[ij] = A[1, 0]
    WuResDim[:, ij] = np.squeeze(np.array(Y - X * A))
        
# Plot WuExtMean
fig = plt.figure()
field = np.ma.masked_all((nlat*nlon,))
field[~mask] = WuExtMeanDim
field = field.reshape(nlat, nlon)
vmax = np.max(field)
vmin = np.min(field)
levels = np.linspace(vmin, vmax, nlev)
cs = map.contourf(x, y, field, levels, cmap=cm.RdBu_r)
plt.title('Mean of the external wind-stress')
map.drawcoastlines()
# draw parallels and meridians.
map.drawparallels(np.arange(0, 81.,10.))
map.drawmeridians(np.arange(-180.,181.,30.))
plt.colorbar(orientation='horizontal')
fig.savefig('tauExtMean_%s.png' % postfix, bbox_inches='tight') 

# Plot std of WuRes
fig = plt.figure()
field = np.ma.masked_all((nlat*nlon,))
field[~mask] = WuResDim.std(0)
field = field.reshape(nlat, nlon)
vmax = np.max(field)
vmin = 0.
levels = np.linspace(vmin, vmax, nlev)
...
# WuExtMean = WuExtMeanDim * toStress * toND
...
#     eof[~mask] = v[:, k]
#     eof = eof.reshape(nlat, nlon)
#     tau += np.tile(np.expand_dims(np.expand_dims(pcrn[:, k], 1), 2), (1, nlat, nlon)) \
...
#     # f = FortranFile('%s/tau_%deofs%s.bin' % (initDir, k+1, postfix), 'w')
#     # for t in np.arange(T):
#     #     f.write_record(tau[t])
#     # f.close()
#     # f = FortranFile('%s/dtaudx_%deofs%s.bin' % (initDir, k+1, postfix), 'w')
#     # for t in np.arange(T):
#     #     f.write_record(dtaudx[t])
#     # f.close()
#     # f = FortranFile('%s/dtaudy_%deofs%s.bin' % (initDir, k+1, postfix), 'w')
#     # for t in np.arange(T):
