...
# Generate the white noise time series and plot the FFTs
print 'Generating white noise principal components...'
pcwn = np.empty((T, nEOF))
for k in np.arange(nEOF):
    pcwn[:, k] = np.random.normal(0, rn[0, k], size=T)
...
# tauMean = np.tile(np.expand_dims(WuResMeanValid, 0), (T, 1, 1)) * ampMean
# dtauMeandx = np.empty(tauMean.shape)
# dtauMeandx[:, :, 1:-1] = (tauMean[:, :, 2:] - tauMean[:, :, :-2]) / dX3Center
# dtauMeandx[:, :, 0] = (tauMean[:, :, 1] - tauMean[:, :, 0]) / dX3Left
# dtauMeandx[:, :, -1] = (tauMean[:, :, -1] - tauMean[:, :, -2]) / dX3Right
# dtauMeandy = np.empty(tauMean.shape)
# dtauMeandy[:, 1:-1] = (tauMean[:, 2:] - tauMean[:, :-2]) / dY3Center
# dtauMeandy[:, 0] = (tauMean[:, 1] - tauMean[:, 0]) / dY3Left
# dtauMeandy[:, -1] = (tauMean[:, -1] - tauMean[:, -2]) / dY3Right
# f = FortranFile('%s/tau_mean_amp%02d%s.bin' \
#                 % (initDir, int(ampMean * 10), dstPostfix), 'w')
# for t in np.arange(T):
#     f.write_record(tauMean[t])
# f.close()
# f = FortranFile('%s/dtaudx_mean_amp%02d%s.bin' \
#                 % (initDir, int(ampMean * 10), dstPostfix), 'w')
# for t in np.arange(T):
#     f.write_record(dtauMeandx[t])
# f.close()
# f = FortranFile('%s/dtaudy_mean_amp%02d%s.bin' \
#                 % (initDir, int(ampMean * 10), dstPostfix), 'w')
# for t in np.arange(T):
#     f.write_record(dtauMeandy[t])
# f.close()

# Write tau with variations
# tauNoNorm = np.zeros(tauMean.shape, tauMean.dtype)
# for k in np.arange(nEOF):
...
#     # Normalize
#     tau =  tauNoNorm / np.tile(tauNoNorm.std(0).mean(), (T, 1, 1))
...
#     dtaudy[:, -1] = (tau[:, -1] - tau[:, -2]) / dY3Right
    # f = FortranFile('%s/tau%s_%deofs_amp%02d%s.bin' \
    #                 % (initDir, noiseType, k+1, int(ampMean * 10),
    #                    dstPostfix), 'w')
    # for t in np.arange(T):
    #     f.write_record(tau[t])
    # f.close()
    # f = FortranFile('%s/dtaudx%s_%deofs_amp%02d%s.bin' \
    #                 % (initDir, noiseType, k+1, int(ampMean * 10),
    #                    dstPostfix), 'w')
    # for t in np.arange(T):
    #     f.write_record(dtaudx[t])
    # f.close()
    # f = FortranFile('%s/dtaudy%s_%deofs_amp%02d%s.bin' % \
    #                 (initDir, noiseType, k+1, int(ampMean * 10),
    #                  dstPostfix), 'w')
    # for t in np.arange(T):
